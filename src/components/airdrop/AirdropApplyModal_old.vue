<template>
  <q-dialog v-model="AirdropApplyModal" persistent>
    <q-card style="display: flex; justify-content: center; width: 900px !important; height: auto; max-width: 850px !important">
      <!-- 팬텀지갑주소 입력 -->
...
export default {
  ...
  methods: {
    ...
    async apply() {
      ...
      let result = await this.checkAddress(this.solanaWalletAddress)
      if(!result) {
        this.isinvalid = true
        return
      }
